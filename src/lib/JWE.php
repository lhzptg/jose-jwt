...
class JWE extends JWT
{
    ...
    private function cipher() {
        switch ($this->header['enc']) {
            case 'A128GCM':
            case 'A256GCM':
                throw new JoseExceptionUnexpectedAlgorithm('Algorithm not supported');
            case 'A128CBC-HS256':
            case 'A256CBC-HS512':
                $cipher = new AES(AES::MODE_CBC);
                break;
            default:
                throw new JoseExceptionUnexpectedAlgorithm('Unknown algorithm');
        }
        switch ($this->header['enc']) {
            case 'A128GCM':
            case 'A128CBC-HS256':
    ...
    private function encryptContentEncryptionKey($public_key_or_secret) {
        ...
        if (!$this->jwe_encrypted_key) {
            throw new JoseExceptionEncryptionFailed('Master key encryption failed');
        }
    }
    ...
    private function deriveEncryptionAndMacKeys() {
        ...
        if (!$this->encryption_key || !$this->mac_key) {
            throw new JoseExceptionDecryptionFailed('Encryption/Mac key derivation failed');
        }
    }
    ...
    private function encryptCipherText() {
        ...
        if (!$this->cipher_text) {
            throw new JoseExceptionDecryptionFailed('Payload encryption failed');
        }
    }
    ...
    private function decryptCipherText() {
        ...
        if (!$this->plain_text) {
            throw new JoseExceptionDecryptionFailed('Payload decryption failed');
        }
    }
    ...
    private function checkAuthenticationTag() {
        if (hash_equals($this->authentication_tag, $this->calculateAuthenticationTag())) {
            ...
            throw new JoseExceptionUnexpectedAlgorithm('Invalid authentication tag');
        }
    }
}
