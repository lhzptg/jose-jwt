...
class JWS extends JWT
{
    ...
    function sign($private_key_or_secret, $algorithm = 'HS256') {
        ...
        if (
            $private_key_or_secret instanceof JWK &&
        ...
        if (!$this->signature) {
            throw new JoseException('Signing failed because of unknown reason');
        }
        return $this;
    }
    ...
    function verify($public_key_or_secret, $alg = null) {
        if ($this->_verify($public_key_or_secret, $alg)) {
            ...
            throw new JoseExceptionVerificationFailed('Signature verification failed');
        }
    }
        ...
        switch ($expected_alg) {
            ...
            case 'HS512':
                if (isset($using_autodetected_alg)) {
                    throw new JoseExceptionUnexpectedAlgorithm(
